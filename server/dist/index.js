...
import { dataDir, distDir, listDevices, loadDevice, saveDevice, registerDevice, recordHeartbeat, applyConfigUpdate, } from "./store.js";
...
function adminAuthHash(u, p) {
    const ADMIN_PASSWORD = adminPassword();
    // single admin user; constant-time-ish compare for home use
    if (u !== "admin")
        return false;
    if (p.length !== ADMIN_PASSWORD.length)
        return false;
    let diff = 0;
    for (let i = 0; i < p.length; i++)
        diff |= p.charCodeAt(i) ^ ADMIN_PASSWORD.charCodeAt(i);
    return diff === 0;
}
async function deviceAuth(req, deviceId) {
    const dir = dataDir();
...
export async function buildApp() {
    ...
    await app.register(fastifyBasicAuth, {
        validate: async (username, password) => {
            if (!adminAuthHash(username, password))
    ...
    app.post("/api/register", {
        ...
        if (!allowAutoRegister())
            return reply.code(403).send({ error: "auto-register disabled" });
    ...
    app.put("/api/admin/devices/:id", { onRequest: app.basicAuth }, async (req, reply) => {
        ...
        const res = applyConfigUpdate(device, patch);
        if (res.error)
